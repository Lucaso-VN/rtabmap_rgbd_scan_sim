...
#include "nav2_ipso_planner/ipso_planner.hpp"

namespace nav2_ipso_planner
{

IPSOPlanner::IPSOPlanner()
  : costmap_(nullptr),
...
void IPSOPlanner::configure(
  ...
  std::shared_ptr<nav2_costmap_2d::Costmap2DROS> costmap_ros)
  {
    if (!initialized_) {
      ...
      // Initialization lifecycle publisher
      ...
      nav2_util::declare_parameter_if_not_declared(
        node_, name + ".w_inertial_min", rclcpp::ParameterValue(0.25));
      node_->get_parameter(name + ".w_inertial_min", w_inertial_min_);

      nav2_util::declare_parameter_if_not_declared(
        node_, name + ".w_inertial_max", rclcpp::ParameterValue(0.75));
      node_->get_parameter(name + ".w_inertial_max", w_inertial_max_);

      nav2_util::declare_parameter_if_not_declared(
        node_, name + ".w_social_lower", rclcpp::ParameterValue(1.2));
      node_->get_parameter(name + ".w_social_lower", w_social_lower_);

      nav2_util::declare_parameter_if_not_declared(
        node_, name + ".w_social_upper", rclcpp::ParameterValue(2.5));
      node_->get_parameter(name + ".w_social_upper", w_social_upper_);

      nav2_util::declare_parameter_if_not_declared(
        node_, name + ".w_cognitive_lower", rclcpp::ParameterValue(1.2));
      node_->get_parameter(name + ".w_cognitive_lower", w_cognitive_lower_);

      nav2_util::declare_parameter_if_not_declared(
        node_, name + ".w_cognitive_upper", rclcpp::ParameterValue(2.5));
      node_->get_parameter(name + ".w_cognitive_upper", w_cognitive_upper_);
      ...
      nav2_util::declare_parameter_if_not_declared(
        node_, name + ".ipso_max_iter", rclcpp::ParameterValue(5));
      node_->get_parameter(name + ".ipso_max_iter", ipso_max_iter_);
  
      // Tạo đối tượng IPSO (file thuật toán IPSO - logic được giữ nguyên)
      g_planner_ = new IPSO(
                      nx_, ny_, resolution_, origin_x_, origin_y_,
                      n_particles_, n_inherited_, pointNum_,
                      w_inertial_min_, w_inertial_max_, 
                      w_social_lower_, w_social_upper_, 
                      w_cognitive_lower_, w_cognitive_upper_,
                      obs_factor_, max_speed_, initposmode_,
                      pub_particles_, ipso_max_iter_, node_);
  
      RCLCPP_INFO(node_->get_logger(), "IPSO planner initialized successfully");
      initialized_ = true;
    } else {
  ...
  void IPSOPlanner::cleanup()
  {
    // Neu can giải phóng tài nguyên thì làm ở đây
  }
  
  void IPSOPlanner::activate()
  {
    plan_pub_->on_activate();
  }
  
  void IPSOPlanner::deactivate()
  {
    plan_pub_->on_deactivate();
  }
  
  nav_msgs::msg::Path IPSOPlanner::createPlan(
  ...
  bool IPSOPlanner::getPlanFromPath(std::vector< std::pair<int, int> >& path, std::vector<geometry_msgs::msg::PoseStamped>& plan)
  {
    plan.clear();
    unsigned int mx = 0, my = 0;
    double wx, wy;
  ...
  void IPSOPlanner::optimizationOrientation(std::vector<geometry_msgs::msg::PoseStamped>& plan)
  {
    if (plan.size() < 2)
      return;
  ...
  bool IPSOPlanner::collision(double x, double y)
  {
    unsigned int mx, my;
  ...
  double IPSOPlanner::distance(double px1, double py1, double px2, double py2)
  {
    return std::hypot(px1 - px2, py1 - py2);
  }
  
  bool IPSOPlanner::pointCircleCollision(double x1, double y1, double x2, double y2, double goal_radius)
  {
    double dist = distance(x1, y1, x2, y2);
    return (dist < goal_radius);
  }
  
  bool IPSOPlanner::isLineFree(const std::pair<double, double> p1, const std::pair<double, double> p2)
  {
  ...
  bool IPSOPlanner::isAroundFree(double wx, double wy)
  {
    unsigned int mx, my;
  ...
  PLUGINLIB_EXPORT_CLASS(nav2_ipso_planner::IPSOPlanner , nav2_core::GlobalPlanner)
